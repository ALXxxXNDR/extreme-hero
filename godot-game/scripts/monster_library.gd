...
# -----------------------------------------------------------------------------
# 3차 v4 피드백 ⑰ — 1스테이지 낮의 「무도발 정적」
# -----------------------------------------------------------------------------
# 사용자 요구 원문: "1스테이지 낮에 선공 몹이 너무 많다. 1스테이지 낮에는 선공 몹이
# 안 나오게 해 줘."
#
# 위 `stage_aggro_gate_ok()`는 이미 1·2스테이지 낮에서 behavior 4를 0으로 만든다
# (`--field-test day_aggro_zero`가 이걸 매 실행 증명한다). 그런데도 체감이 남는 이유는
# 게이트가 보는 축이 **종의 행동 유형**인 반면, 플레이어가 "선공"이라고 부르는 것은
# **안 건드렸는데 달려오는 몸짓**이기 때문이다. 그 몸짓은 behavior가 아니라 습성에서
# 나온다 — 들멧돼지(behavior 2 · habit `guard`)가 `enemy.gd::_update_field_aggro()`의
# 텃세 분기에서 165px 안이면 스테이지도 시간대도 안 보고 켰다.
#
# 마릿수는 적다. 굴림 가중으로는 1스테이지 낮 풀의 약 1/3이지만 무리(herd)가 한 번에
# 3~5기씩 서기 때문에, 실제로 서 있는 개체 중에서는 3~9%다(`--field-test` 실측 —
# 27기 필드에 1~2기). 그런데도 체감이 컸던 이유는 **그 1~2기가 1스테이지 낮에서
# 유일하게 먼저 달려오는 것**이었기 때문이다. 나머지 세 종은 전부 맞아야 반응한다.
#
# 그래서 게이트가 하나 더 필요하다. 위 게이트가 **스폰**을 막는다면 이쪽은
# **런타임 도발 판정**을 막는다. 두 함수는 축이 다르므로 합치지 않는다
# (`stage_aggro_gate_ok`의 시그니처·반환값은 호출부가 많아 그대로 둔다).
#
# 규칙 한 줄: **1스테이지 낮에는 도발당하지 않은 개체가 먼저 덤비지 않는다.**
#   · 반격은 그대로다 — `provoke()`가 켠 aggro는 이 게이트가 손대지 않는다.
#     맞으면 덤비는 것은 선공이 아니라 반격이고, 피드백은 선공만 지적했다.
#   · 밤은 손대지 않는다. 밤은 행동 유형과 무관하게 전원 습격이 설계다.
#   · 2스테이지부터는 종전대로다. 텃세의 텃세다움은 거기서부터 산다.
#
# ⚠️ 상한이 상수 하나인 이유: §5.4는 "1·2스테이지 낮 선공 0"을 말하는데 지금 값은 1이다.
#    2스테이지까지 넓히는 판단이 서면 이 상수만 2로 올리면 되고, `--field-test`의
#    `peace_gate` 진단이 그 순간 세 값 중 마지막 칸으로 바뀐 것을 바로 보여 준다.
const STAGE_DAY_PEACE_MAX := 1

## 이 스테이지·시간대가 「무도발 정적」인가. true면 맞지 않은 개체는 먼저 덤비지 않는다.
static func stage_day_peaceful(stage: int, night: bool) -> bool:
	return not night and stage <= STAGE_DAY_PEACE_MAX
