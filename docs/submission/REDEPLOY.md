...
저장소 루트(`/Users/moomi/orca/projects/NHN`)에서 아래 한 줄만 실행하면 **두 배포처가 한 번에 갱신됩니다.**
...
이 게임의 웹 빌드는 스레드를 쓰기 때문에(`variant/thread_support=true`) 브라우저가 `SharedArrayBuffer`를 허용해야 하고, 그러려면 서버가 `Cross-Origin-Opener-Policy: same-origin`과 `Cross-Origin-Embedder-Policy: require-corp` 두 헤더를 보내 줘야 합니다. Vercel은 `build/web/vercel.json`으로 이 헤더를 직접 지정하지만, **GitHub Pages는 커스텀 헤더를 설정할 방법이 없습니다.** 그래서 서비스 워커가 헤더를 대신 붙여 주는 coi-serviceworker를 함께 올립니다. 이미 헤더가 있는 환경(=Vercel)에서는 아무 일도 하지 않고 지나가므로, 두 배포처에 같은 산출물을 올려도 안전합니다.
...
- 빌드 내용이 이전과 똑같으면 GitHub Pages 푸시는 건너뜁니다.
